...
export default function AdvantagesSection() {
  return (
    <section id="advantage" className="w-full bg-gradient-to-b from-[#9DC183] to-[#8bb572] text-white py-12 md:py-20">
      <div className="container mx-auto px-4 max-w-7xl">
        <div className="text-center mb-8 md:mb-12 max-w-2xl mx-auto">
          <h2 className="text-2xl md:text-4xl font-bold mb-4 tracking-tight">Kayana Roasting is Different</h2>
        ...
        <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
          {advantages.map(({ icon: Icon, title, description, points }) => (
            <div
              key={title}
              className="bg-white/20 p-5 md:p-6 rounded-xl backdrop-blur-sm border border-white/20 hover:border-[#893F26]/40 transition-all group"
            >
              <div className="flex items-center mb-4">
                <div className="bg-[#893F26] h-12 w-12 rounded-full flex items-center justify-center shadow-lg group-hover:shadow-[#893F26]/30 mr-4">
                  <Icon className="h-6 w-6 text-white" strokeWidth={2} />
                </div>
                                  <h3 className="text-lg md:text-xl font-bold group-hover:text-[#893F26] transition-colors">{title}</h3>
              </div>
                ...
                {points.map((point, index) => (
                  <li key={index} className="flex items-start gap-2">
                    <div className="mt-1 flex-shrink-0">
                      <svg className="h-4 w-4 text-[#893F26]" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M5 13l4 4L19 7" />
                      </svg>
                    </div>
                    <span className="text-xs md:text-sm text-white/90">{point}</span>
                  </li>
                ))}
              </ul>
            </div>
